...
func makeTests() []*TestGroup {
	...
	tests := []*TestGroup{
		...
		testgroup("Ech",
			...
			tc("Create a HTTPS record",
				https("@", 0, "example.com.", "")),
			tc("Change priority, add params",
				https("@", 1, "example.com.", "alpn=h2,h3")),
			tc("Change target",
				https("@", 1, "other.com.", "alpn=h2,h3")),
			tc("Change params",
				https("@", 1, "other.com.", "alpn=h3 port=80")),
			tc("Add an ECH key",
				https("@", 1, "other.com.", "alpn=h3 port=80 ech=some+base64+encoded+value///")),
			// tc("IGNORE ECH",
			// 	https("@", 1, "other.com.", "alpn=h3 port=80 ech=IGNORE")).ExpectNoChanges(),
			// tc("Ignore the ECH key while changing other values",
			// 	https("@", 1, "other.com.", "alpn=h2,h3 port=80 ech=IGNORE")),
			tc("Change the ECH key and other values",
				https("@", 1, "example.org.", "port=80 ipv4hint=127.0.0.1 ech=another+base64+encoded+value")),
			tc("Another domain with a different ECH value",
				https("ech", 1, "yetanother.com.", "ech=third+base64+encoded+value//")),
		),

		testgroup("SVCB",
			requires(providers.CanUseSVCB),
			tc("Create a SVCB record",
				svcb("@", 1, "test.com.", "port=80")),
			tc("Change SVCB priority",
				svcb("@", 2, "test.com.", "port=80")),
			tc("Change SVCB target",
				svcb("@", 2, ".", "port=80")),
			tc("Change SVCB params",
				svcb("@", 2, ".", "port=99")),
			tc("Change SVCB remove params",
				svcb("@", 0, ".", "")),
		),
